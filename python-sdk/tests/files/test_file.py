...
@patch("astro.files.base.create_file_location")
def test_is_binary(mocked_location, filetype):
    """Test if the file is of text/binary format"""
    mocked_location.return_value.paths = [filetype[1]]
    assert File(filetype[1]).get_first().is_binary() == filetype[0]
...
#     mocked_location, mocked_smart_open, filetype, locations, type_method_map_fixture
# ):
...
#         mocked_location.return_value.paths = [path]
#         File(path).get_first().create_from_dataframe(df=df)
#         mocked_smart_open.assert_called()
#         kwargs = mocked_smart_open.call_args.kwargs
#         args = mocked_smart_open.call_args.args
#         if kwargs["transport_params"]:
