...
@pytest.mark.parametrize(
    ...
    [
        (DEFAULT_CONN_ID, "mssql+pymssql://localhost:1433"),
        (CUSTOM_CONN_ID, "mssql+pymssql://sa:Passw%40rd@localhost:1433/pkvdb"),
    ],
...
def test_load_pandas_dataframe_to_table(database_table_fixture):
    ...
    statement = f"SELECT * FROM {database.get_table_qualified_name(table, fully_qualified = True)};"
    response = database.run_sql(statement)
