...
def generate_cluster_identifier(len: int = 15):
    return ''.join(random.choices(string.ascii_lowercase + str(string.digits), k=15))
...
def describe_cluster_status(cluster_identifier: str):
    ...
    return response['Clusters'][0]['ClusterStatus'], response['Clusters'][0].get('Endpoint', {}).get('Address', None)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--action",
        type=str,
        help="accepted values are create/delete",
        required=True
    )
    parser.add_argument(
        "--cluster_id",
