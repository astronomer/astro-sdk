...
def test_sql_files_dag(sql_files_dag, sql_files_dag_with_parameters, sql_file, sql_file_with_parameters):
    ...
    assert sql_files_dag.sorted_workflow_files() == [sql_file]
    assert sql_files_dag_with_parameters.sorted_workflow_files() == [sql_file_with_parameters]
...
def test_sql_files_dag_with_cycle(sql_files_dag_with_cycle):
    ...
    with pytest.raises(DagCycle):
        assert sql_files_dag_with_cycle.sorted_workflow_files()
