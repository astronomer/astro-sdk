...
compliant_dag_first_part = """# NOTE: This is an auto-generated file. Please do not edit this file manually.
from pathlib import Path
...
compliant_dag_single_task_first_part = """# NOTE: This is an auto-generated file. Please do not edit this file manually.
from pathlib import Path
...
"""
compliant_dag_single_task_second_part = """
    a = naql.nominal_transform_file(
        file_path=f"{DAGS_FOLDER}/include/basic/a.sql",
        parameters={},
        conn_id="my_test_sqlite",
        op_kwargs={
...
@freeze_time(datetime.datetime(2016, 12, 22, 3, 49, 0))
def test_generate_dag_single_task_run_black_compliant(root_directory, dags_directory):
    dag_file = generate_dag(
        directory=root_directory, dags_directory=dags_directory, generate_tasks=True, tasks_to_execute=["c"]
    )
    computed = dag_file.read_text()

    first_part = compliant_dag_single_task_first_part
    if airflow_version() < Version("2.4"):
        first_part = compliant_dag_single_task_first_part.replace("schedule", "schedule_interval")

    assert computed.startswith(first_part)
    assert computed.endswith(compliant_dag_single_task_second_part)
