...
class TestPostgresMergeOperator(unittest.TestCase):
    ...
    def test_merge_basic_single_key(self):
        ...
        hook.run(
            sql=f"ALTER TABLE {test_utils.DEFAULT_SCHEMA}.merge_test_1 ADD CONSTRAINT airflow UNIQUE (list)"
        )
        a = aql.merge(
        ...
        df = hook.get_pandas_df(
            sql=f"SELECT * FROM {test_utils.DEFAULT_SCHEMA}.merge_test_1"
        )
    ...
    def test_merge_basic_ignore(self):
        hook = PostgresHook(schema="pagila", postgres_conn_id="postgres_conn")
        hook.run(
            sql=f"ALTER TABLE {test_utils.DEFAULT_SCHEMA}.merge_test_1 ADD CONSTRAINT airflow UNIQUE (list,sell)"
        )
