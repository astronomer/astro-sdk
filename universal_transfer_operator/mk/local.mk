...
.DEFAULT_GOAL:= help
SHELL := /bin/bash
PROJECT_NAME := universal-transfer-operator
...
ifdef db
    @cd .. && $(PYTEST) -s --cov --cov-branch --cov-report=term-missing -m "$(db)"
else
    @cd .. && $(PYTEST) -s --cov --cov-branch --cov-report=term-missing
endif

unit: virtualenv config ## Run unit tests
    @cd .. && $(PYTEST) -s --cov --cov-branch --cov-report=term-missing -m "not integration"

integration: virtualenv config  ## Run integration tests
    @cd .. && $(PYTEST) -s --cov --cov-branch --cov-report=term-missing -m integration
