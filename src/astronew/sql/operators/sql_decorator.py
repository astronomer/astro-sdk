...
from astronew.databases import get_db_from_table
from astronew.table import Table, TableHandler, TempTable, create_unique_table_name
...
class SqlDecoratedOperator(DecoratedOperator, TableHandler):
    ...
    def execute(self, context: Dict):
        ...
        db = get_db_from_table(
            Table(conn_id=self.conn_id, schema=self.schema, role=self.role)
        )
        context = db.add_templates_to_context(
            parameters=self.parameters, context=context
        )
        if context:
            self.sql = self.render_template(self.sql, context)
        # TODO (kaxil): Add support for snowflake
        # self._process_params(db=db_type)

        output_table_name: str = ""
        # Create DEFAULT SCHEMA if not created.
        db.create_default_schema_if_needed()
        ...
        if not self.raw_sql:
            ...
            if not self.output_table or type(self.output_table) == TempTable:
                ...
                output_table_schema = self.output_table.schema

            db_for_output_table = get_db_from_table(
                Table(
            ...
            db_for_output_table.create_schema_if_needed()
            db_for_output_table.drop_table()
            self.sql = self.create_temporary_table(
                self.sql, db_for_output_table.qualified_name
            )
        else:
        ...
        query_result = db_for_output_table.run_sql(self.sql, self.parameters)
    ...
    # TODO (kaxil) - Uncomment when adding Snowflake support
    # def _process_params(self, db):
    ...
    def convert_op_arg_dataframes(self):
        ...
        for i, arg in enumerate(self.op_args):
            if isinstance(arg, pd.DataFrame):
                ...
                db = output_table.get_database()
                db.load_pandas_dataframe(pandas_dataframe)
