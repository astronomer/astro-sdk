...
class AgnosticLoadFile(BaseOperator):
    ...
    def execute(self, context):
        ...
        self.normalize_config = output_db.pandas_populate_normalize_config(
        ...
        return self.load_using_pandas(output_db, context, paths, transport_params)

    def load_using_pandas(self, db, context, paths, transport_params):
        """Loads csv/parquet table from local/S3/GCS with Pandas.

        Infers SQL database type based on connection then loads table to db.
        """
        self._configure_output_table(db, context)
    ...
    def _configure_output_table(self, db, context):
        ...
        if isinstance(self.output_table, TempTable):
            self.output_table = self.output_table.to_table(
                db.generate_table_name(context=context)
            )
        if not self.output_table.table_name:
            self.output_table.table_name = db.generate_table_name(context=context)
